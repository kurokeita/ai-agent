...
6. **Lint Markdown Files (if applicable):**
   - If the task added or modified any markdown files, you MUST lint them using the targeted linting command:

     ```bash
     pnpm lint:md <file_path>
     ```

   - If linting issues are found, use the auto-correction command to fix them:

     ```bash
     pnpm markdownlint <file_path> --fix
     ```

   - Confirm that the files now pass linting.
...
- [ ] Markdown files pass linting (using `pnpm lint:md`)
